...
2. Ensure you pass `user_id` in your `trackLead()` call (required for payment attribution)
...
**Note:** In debug mode, `trackLead()` sends a test event only — no actual lead record is created.
