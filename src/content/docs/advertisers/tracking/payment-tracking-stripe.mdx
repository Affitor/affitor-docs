...
:::tip[Using the direct Sale API instead?]
If you're not using Stripe Checkout, you can track sales via `POST /api/v1/track/sale`. Amounts must be in **cents** (e.g., `$99.99` = `amount_cents: 9999`). See the [3-Step Integration Guide](/advertisers/tracking/quickstart-integration/#step-3--track-sales) for the full API reference.
:::
