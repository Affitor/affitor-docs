...
:::tip[API uses cents]
When using the [Server-Side Sale API](/advertisers/tracking/quickstart-integration/#step-3--track-sales), amounts must be in **cents** as a positive integer. For example, `$99.99` = `amount_cents: 9999`. Stripe metadata does not require this — Stripe handles amounts natively.
:::
