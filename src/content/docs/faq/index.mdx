...
3. **Visit with a `?aff=` parameter** — Pageview tracking activates when a visitor arrives via an affiliate link, e.g. `https://yoursite.com?aff=TEST123`.
...
- **Script not loaded** — Check DevTools → Network for the tracker script request. A 404 means the script URL is wrong. Make sure you're using the correct URL:
  ...
  - UAT/Staging: `https://uat-affitor-cms.vanilla-ott.com/js/affitor-tracker-uat.js`
...
- **Wrong tracker script URL** — Development may use the UAT script (`affitor-tracker-uat.js`). Production must use `affitor-tracker.js`:
  ...
  <script src="https://uat-affitor-cms.vanilla-ott.com/js/affitor-tracker-uat.js" ...>
  ```
