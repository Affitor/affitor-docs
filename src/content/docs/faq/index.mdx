...
### Cookies not being set (`customer_code` missing)

If you check DevTools → Application → Cookies and don't see a `customer_code` cookie:
...
1. **Pageview tracker must work first** — The `customer_code` cookie (set by pageview tracking) is needed to link leads to affiliates. Complete and verify pageview tracking before testing lead tracking.
2. **`trackLead()` not called** — Ensure `trackLead()` is called *after* successful signup, not before or during form submission.
3. **`user_id` is missing** — The `user_id` field is **required**. Without it, lead events will not be processed for payment attribution.

   ```js
   // ✅ Correct
   affitor?.trackLead({ email: 'user@example.com', user_id: 'user_123' });

   // ❌ Missing user_id — will not attribute payments
   affitor?.trackLead({ email: 'user@example.com' });
   ```
...
5. **Check console output** — With debug mode on, you should see `[Affitor] trackLead called with: {...}` and `[Affitor] Lead event tracked`.
