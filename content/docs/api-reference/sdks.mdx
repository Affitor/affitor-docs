---
title: SDKs
description: Typed client libraries for browser tracking (@affitor/tracker) and server-side conversion reporting (@affitor/node).
---
...
Two packages cover the full attribution chain: the browser SDK captures clicks and leads on the frontend; the Node SDK reports conversions from your backend.

<FeatureCards>
  <FeatureCard icon="monitor" title="@affitor/tracker" href="#affitortracker-browser">Browser SDK — captures affiliate clicks, persists attribution in a first-party cookie, and reports lead events.</FeatureCard>
  <FeatureCard icon="server" title="@affitor/node" href="#affitornode-server">Node SDK — reports leads, sales, clicks, and refunds from your backend with Bearer auth.</FeatureCard>
</FeatureCards>

---

## @affitor/tracker (Browser)
...
```html
<script
  src="https://cdn.affitor.com/affitor-tracker.js"
...
```ts
import { init, signup } from '@affitor/tracker';
...
```ts
// lib/affitor.ts — client-side init, called once from _app.tsx
import { init, signup, getData } from '@affitor/tracker';

export function initAffitor() {
  init({ programId: Number(process.env.NEXT_PUBLIC_AFFITOR_PROGRAM_ID) });
}

// After successful signup
export async function trackSignup(userId: string, email: string) {
  await signup(userId, email);
  console.log('Attribution state:', getData());
}
```
...
Track a click server-side. Click tracking is usually handled in the browser by `@affitor/tracker`; use this only for server-rendered flows or redirect-based affiliate links.
...
- [Lead Tracking Guide](/docs/advertisers/tracking/lead-tracking-signup) — implementation walkthrough
