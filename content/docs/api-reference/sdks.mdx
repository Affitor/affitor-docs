...
| `currency` | `string` | No | `USD`, `EUR`, or `VND`; other codes are not supported. Defaults to `USD`. Amounts are not converted between currencies: commissions and platform fees are calculated on the amount as sent, and invoices are issued in USD. |
...
- [Track Click](/api-reference/track-click) — raw HTTP endpoint reference
- [Track Lead](/api-reference/track-lead) — raw HTTP endpoint reference
- [Track Sale](/api-reference/track-sale) — raw HTTP endpoint reference
- [Track Refund](/api-reference/track-refund) — raw HTTP endpoint reference
