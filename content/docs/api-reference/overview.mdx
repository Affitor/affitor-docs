...
All endpoints return JSON. A successful tracking call includes `"success": true`; the readiness endpoint returns its verdict object instead, with no `success` field. On an error, `error` is an object — never a plain string.
...
There is **no** `/api/tracking/` alias for click, lead, sale, or refund. The four `/api/v1/track/*` paths above are the only ones that accept tracking calls from your backend.

The `/api/tracking/` paths that do exist back the Affitor dashboard, not a server-to-server integration. They authenticate with a signed-in dashboard session and require an active workspace membership on the program — taken from the path, or from `program_id` in the body on the two `POST` routes that have no path parameter. A Bearer program API key is not accepted on any of them:
