...
There are two shapes, and which one you get depends on the endpoint.

### Tracking endpoints

`POST /api/v1/track/click`, `/lead`, `/sale`, and `/refund` return the platform envelope. Read `error.message`:
...
`data` is `null` on every error. `error.status` repeats the HTTP status, and `error.name` is the error class for that status — `BadRequestError` (400), `UnauthorizedError` (401), `NotFoundError` (404), `ConflictError` (409), `InternalServerError` (500). This shape carries no stable machine code, so match on the HTTP status and, where you need the specific cause, on the `error.message` strings listed below.

### Readiness endpoint

`GET /api/v1/programs/me/readiness` returns the typed agent envelope instead. It has no `data` key, and it carries a stable `error.code` you can branch on:
...
One error crosses the boundary: [`api_key_rotation_required`](#api_key_rotation_required). A program API key past its rotation deadline is refused with the typed envelope on **every** endpoint that key authenticates, tracking endpoints included.

So read `error.code` first and fall back to `error.message` when it is absent:
...
The success body is identical in both modes, so a `2xx` on its own does not prove your key was read. The one exception is a program configured with `lead_auth: server_required`, where a browser-mode lead comes back with `"provisional": true` and settles only once your backend sends the server-side lead.
...
This is the one error the tracking endpoints answer with the [typed agent envelope](#readiness-endpoint) rather than the platform one: no `data` key, and an `error.code` to branch on. Every other tracking error uses the platform envelope and carries no `code`.
