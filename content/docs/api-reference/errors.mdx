...
The success body is identical in both modes, so a `2xx` on its own does not prove your key was read. The one exception is a program configured with `lead_auth: server_required`, where a browser-mode lead comes back with `"provisional": true`. Such a lead settles on either of two events, whichever comes first: your backend sends the server-side lead, or the customer's first verified Stripe payment arrives — Affitor creates the lead from the `checkout.session.completed` webhook through the same path the lead endpoint uses.
...
The synthetic verification chain has a second, separate cap of 10 runs per program per hour. It reports the same `rate_limited` code on `POST /api/v1/cli/test-event`, with its own `retry_after_seconds`.
...
if (!response.ok) {
  ...
  // (api_key_rotation_required here); the platform envelope has message only.
  const detail = body.error?.code
    ? `${body.error.code}: ${body.error.message}`
    : body.error?.message;
