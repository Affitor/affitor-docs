...
All error responses return JSON with an `"error"` string. There is no wrapper envelope on error paths — with one exception, [`api_key_rotation_required`](#api_key_rotation_required), where `error` is an object and you should read `error.code` instead.
...
This is the one error on this page returned as a nested `error` object instead of a flat string — read `error.code`, not `error`, when handling this response.
...
if (!response.ok) {
  ...
  // `error` is a string on every error but `api_key_rotation_required`,
  // where it is an object carrying the code you branch on.
  const detail =
