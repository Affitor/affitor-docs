...
`data` is `null` on every error in this envelope. `error.status` repeats the HTTP status, and `error.name` is the error class for that status — `BadRequestError` (400), `UnauthorizedError` (401), `NotFoundError` (404), `ConflictError` (409), `InternalServerError` (500). It carries no stable machine code, so match on the HTTP status and, where you need the specific cause, on the `error.message` strings listed below.
