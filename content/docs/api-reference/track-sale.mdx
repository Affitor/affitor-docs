...
<ParamList>
  ...
  <ParamField name="currency" type="string">
    `USD`, `EUR`, or `VND`; other codes are not supported. Defaults to `USD`. Amounts are not converted between currencies: commissions and platform fees are calculated on the amount as sent, and invoices are issued in USD.
  </ParamField>
...
<ResponseTabs items={[
  ...
  "error": {
    "status": 409,
    "name": "ConflictError",
    "message": "Duplicate transaction_id. This sale has already been recorded.",
    "details": {}
  }
}`
  },
