---
title: "Node / Express"
description: "Server-side Affitor integration with affitor-node — track signups and sales from your Express backend."
---

This guide wires Affitor into a Node.js / Express app. Because Express is a pure server runtime, the click is captured by a frontend script tag, then the `affitor_click_id` is forwarded to your server so all tracking calls — lead and sale — happen server-side with `affitor-node`.

:::note
The `affitor-node` package is **Beta**. The documented happy-path works; report issues on GitHub.
:::
...
- `affitor-node` installed in your backend
...
:::note
`customer_key` in the raw API, `customerExternalId` in `affitor-node`, and `affitor_customer_key` in Stripe metadata are all **the same value** — your stable internal user ID. Keep it consistent.
:::
