...
**The `affitor_click_id` is a first-party browser cookie.** A server-side callback has no access to it. If you call `affitor-node trackLead` from `events.createUser` without passing a `clickId`, the lead is recorded but has no partner to attribute to — the click attribution is lost.
