...
- [CLI Command Reference](/advertisers/cli/commands/) — `npx affitor onboard`, the one-shot equivalent of the MCP flow.
...
<NextStep title="Prefer a single command?" description="npx affitor onboard runs detect → install → inject → verify in one shot — the CLI equivalent of the MCP flow." href="/advertisers/cli/commands" />
