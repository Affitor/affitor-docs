...
|-------|------|----------|-------------|
| `transaction_id` | string | **Yes** | The `transaction_id` originally passed to `POST /api/v1/track/sale`. Used to locate the sale record. |
| `refund_amount_cents` | integer | No | Refund amount in cents. Omit or set to `0` to trigger a full refund. Must be a positive integer when provided. |
| `refund_reason` | string | No | Human-readable reason for the refund. Recorded on the commission and included in partner/advertiser notifications. |
...
- [Payment Tracking](/advertisers/tracking/payment-tracking-stripe/) — Stripe webhook alternative
