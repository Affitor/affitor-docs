...
```json
{
  "data": {
    ...
    "statistics": {
      ...
      "payments": {
        "paymentTrackingMode": "stripe_connect",
        "methodReady": true,
...
| `data.stepStatuses.payments` | string | `"complete"` when the program's Stripe connection `stripeConnectionStatus` is `"connected"`, **or** when the program uses server-to-server sale tracking (`paymentTrackingMode` is `"s2s"`) |
...
| `data.statistics.payments.paymentTrackingMode` | string | `"stripe_connect"` (default) or `"s2s"` |
| `data.statistics.payments.methodReady` | boolean | `true` when the payments step is complete |
| `data.statistics.payments.stripeConnectionStatus` | string | Raw Stripe connection status of the program |
