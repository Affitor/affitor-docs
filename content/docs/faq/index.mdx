...
### When do I pay?
In the **Bill Flow** model, Affitor bills you through its invoice workflow after tracked conversions are validated. Confirm billing timing directly with the Affitor team if it affects your launch decision.
...
With Stripe Bill Flow, Affitor attributes the sale from metadata and webhook events. For subscriptions, renewals also require `subscription_data.metadata` to be populated.
