...
The dashboard displays the key masked, and no read returns the full value. It is revealed exactly once — in the response to generating or regenerating it. If you no longer have the current key stored anywhere, you can't recover it from the dashboard; regenerate it instead (Step 3).
