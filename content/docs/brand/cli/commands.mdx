...
Two different credentials are in play, and mixing them up is the most common reason a command fails.
...
In `--json` mode, a failure prints a machine code on `error`.

Do not branch on the exit code alone. `affitor status` exits `0` even when the API refuses the key, and `affitor onboard` exits `0` even when verification does not pass — read `error` from the JSON output, and `integration_verified` from `onboard --json`, to know what happened.

| Condition | `--json` code | What to do |
|---|---|---|
| Not logged in | `not_logged_in` | `affitor init` and `affitor programs` need an account session. Run `affitor login`, then re-run the command |
| No config in this directory | `no_config` | Run the command from the project root where you ran `affitor init`, or run `affitor init` to create a program here |
| Already configured | `already_configured` | `affitor init` found an existing `.affitor/config.json` and changed nothing. Use `affitor status` to check that program. Do not run `init` to recover a key — it only ever creates a new program |
| API key expired or invalid | no code — `error` is the API message, with the HTTP status on `status` | The key you sent matches no program, or it was replaced. Compare it against the value you stored when it was issued; if you no longer hold that value, ask a workspace owner to regenerate the key under Settings → API Key and deploy the new one |
| Invalid event type | `invalid_event_type` | `affitor test` takes `click`, `lead`, or `sale` |
| No API key for `onboard` | `no_api_key` | Pass `--api-key`, set `AFFITOR_API_KEY`, or run from a directory with `.affitor/.env` |
| Network error | `network_error` | The request never reached the API — check connectivity and retry |
