...
| `--duration-months <months>` | Recurring commission duration in months. `0` means one-time (first purchase only), not lifetime. For lifetime, leave the flag out | Prompt pre-fills 12, and is only asked for the recurring types |
...
In `affitor` 0.4.1 the duration prompt and the summary printed after `init` label `0` as "lifetime". That label is wrong: Affitor stores `0` as a one-time commission that pays only on the first purchase. To create a lifetime commission, run `init` with `--no-interactive` and leave out `--duration-months`, or change the duration to lifetime in the dashboard after the program is created.
...
- **`onboard` does not report a lost connection as an error.** If every readiness poll fails, it still prints a summary with `integration_verified: false` and no `error` field. Not verified is not the same as verified-as-broken — see [what that verdict means](#example-agent--non-interactive).
