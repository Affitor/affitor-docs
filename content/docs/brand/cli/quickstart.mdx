...
The commission flags are saved on the program, but partners are paid by the program's Default group commission: set it in the dashboard after `init` (see [CLI commands](/brand/cli/commands)).
