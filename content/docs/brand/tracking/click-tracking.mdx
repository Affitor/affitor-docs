...
**Next.js 13+ App Router:** `layout.tsx` is a Server Component by default. `@affitor/sdk` uses browser APIs and cannot run in a Server Component. Wrap your app in `AffitorProvider` from a client component and render it once in your layout.
...
// wrap {children} with <Providers> once in app/layout.tsx
```

Child components read the loaded tracker with `useAffitor()` from `@affitor/sdk/react`. It returns `null` while the tracker is loading and during server rendering.
...
`loadAffitor()` loads the tracker script once and resolves to the tracker instance (the same object as `window.affitor`). It resolves to `null` during server rendering.

### Astro

Astro isn't React, so the React `<AffitorProvider>` shown above doesn't apply. Load the tracker from a client `<script>` in your shared layout:
...
With React, pass `debug` to the provider: `<AffitorProvider programId="YOUR_PROGRAM_ID" debug>`.
