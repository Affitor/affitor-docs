...
<VerifySuccess
  browser={["Your inbox shows the invitation with your program's terms in the body and a working Accept button"]}
  dashboard={["The Partners table shows the address with an Invited badge and a 30-day expiry; after you accept from the email, the row flips to Active and the partner detail shows a tracking link"]}
  ifNotWorking={["A failed delivery is flagged on the invite row with the reason — hover the row and use Resend invite once fixed"]}
/>
