---
title: "Create Your Advertiser Account"
description: "Step-by-step guide to registering as an advertiser on Affitor and setting up your account to start working with affiliate partners."
---


**Get started with Affitor in minutes.** Creating an advertiser account is the first step to accessing our network of affiliate partners and growing your business through performance-based marketing.

---

## What You'll Learn

- How to register as an advertiser on Affitor
- What information you'll need to provide
- How to access your advertiser dashboard
- Next steps after account creation

---

## Prerequisites

Before starting, ensure you have:

- ✅ A business or company email address
- ✅ Basic information about your business
- ✅ Products or services to promote

> 💡 **Note**: You don't need a Stripe account yet - we'll set that up in a later step. Focus on creating your account first.

---

## Registration Steps

Follow these simple steps to create your advertiser account:

### Step 1: Visit Affitor Management Portal

Navigate to the Affitor management portal:

🔗 **[https://affitor.com/affiliate-management/welcome-page](https://affitor.com/affiliate-management/welcome-page)**

### Step 2: Select "Get started"

On the landing page, click the **"Get started"** button. This will direct you to the role selection page.

### Step 3: Select "I'm a Company"

Click the **"I'm a Company"** button. This will direct you to the advertiser registration flow.

> ⚠️ **Important**: Make sure you select "I'm a Company" (Advertiser) and not "I'm a Partner" (Affiliate). Companies are businesses looking to work with affiliates, while Partners are marketers looking to promote products.

### Step 3: Login with Company Email

Use your **company or business email address** to create your account. This ensures:

- Professional communication with partners
- Proper account verification
- Access to business-level features

> ✅ **Best Practice**: Use an email like `partnerships@yourcompany.com` or `affiliates@yourcompany.com` rather than a personal email. This makes it easier to manage team access later.

### Step 4: Account Created!

Once verified, your advertiser account is **now active**. You'll be redirected to your advertiser dashboard.

### Step 5: Complete Initial Setup Wizard

As a first-time user, you'll be automatically redirected to a **setup wizard** to configure your initial affiliate program. This one-time setup will ask you to provide:

- **Program Name** - The name of your affiliate program (e.g., "Acme Partners Program")
- **Website URL** - Your business website where products/services are hosted
- **Logo** - Upload your company or program logo for branding

> ✨ **One-Time Setup**: This wizard only appears for new accounts. Once completed, you'll have full access to your advertiser dashboard and can manage all settings from there.

After completing the setup wizard, you'll gain access to the full advertiser dashboard where you can manage commissions, track performance, and work with affiliate partners.

---

## What Happens Next?

After creating your account, you're ready to:

1. **Create Your Affiliate Program** - Define your commission structure and program details
2. **Connect Stripe** - Link your Stripe account for payment processing
3. **Add Tracking** - Install the tracking script on your website
4. **Invite Partners** - Start working with affiliate marketers

:::tip[Skip the wizard — use the CLI]
You can also create your program and configure everything from the terminal:

```bash
npx affitor init
```

This creates your program, generates an API key, and outputs ready-to-use tracking snippets. See the [CLI Quickstart](/docs/advertisers/cli/quickstart/).
:::

---

## Account Information

After registration, you can update your account information including:

- Company name and logo
- Business website
- Contact information
- Team member access
- Notification preferences

---

## Frequently Asked Questions

<details>
<summary>Can I use a personal email address?</summary>

Yes, but we recommend using a business email for professionalism and easier team management. You can update your email later in account settings.

</details>

<details>
<summary>Is there a cost to create an account?</summary>

No, creating an account is completely free. You only pay platform fees and partner commissions when you make sales through the platform.

</details>

<details>
<summary>Do I need to provide payment information now?</summary>

No, you don't need to provide payment information during registration. You'll connect your Stripe account later when setting up your program.

</details>

---

## Next Steps

<div className="next-steps-grid">

**Continue Setup:**
- [Setup Your Program](/docs/advertisers/quickstart/setup-program) - Create your affiliate program
- [Connect Stripe](/docs/advertisers/tracking/payment-tracking-stripe) - Link your payment account
- [Define Commission Structure](/docs/advertisers/quickstart/define-commission) - Set your commission rates

**Learn More:**
- [How It Works](/docs/getting-started/how-it-works) - Understand the complete flow
- [Pricing Model](/docs/getting-started/pricing-performance-model) - Learn about fees

**Need Help?**
- [Contact Support](/docs/support/contact)
- [View FAQ](/docs/faq)

</div>
