...
**Script tag (HTML, WordPress, no-build sites):**
...
Set `debug={true}` (SDK) or `data-affitor-debug="true"` (script tag) during testing to see tracking events in your browser console.
...
|--------|--------------|----------|
| **Server-Side Sale API** | Your backend sends completed sale events to Affitor | Any payment provider or custom checkout |
| **Bill Flow (Stripe metadata)** | You collect payments; Affitor tracks via Stripe metadata and invoices fees | Existing Stripe checkout |
