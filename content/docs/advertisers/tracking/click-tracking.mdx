...
Track referred signups the same way — call `signup()` from a client `<script>` after the user authenticates. See [Lead Tracking (Signup)](/advertisers/tracking/lead-tracking-signup/).
