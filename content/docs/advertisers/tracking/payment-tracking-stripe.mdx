...
Some products charge customers per usage event — no subscription object, just independent checkout sessions. Examples: API credit top-ups, usage-based batches billed separately, metered add-ons.
...
The first purchase is straightforward — if the customer clicked an affiliate link recently, `affitor_click_id` is still in their browser cookie.

Later purchases may arrive weeks or months after the last click. The cookie may be gone. Attribution still works if you pass a stable `affitor_customer_key` in every checkout metadata. Affitor looks up the customer record created at signup and links all subsequent purchases to the same partner.
...
Cross-purchase attribution requires an affiliate-customer record, which is created when the signup tracker fires at user registration:

```js
affitor.signup({ customer_key: currentUser.id });
```

If signup tracking was not set up or did not fire for a given user, no customer record exists and later purchases will not be attributed.

### Attribution does not expire

Once an affiliate-customer record is linked to a partner, the link persists indefinitely. There is no time-based cut-off on the customer-to-partner relationship.
