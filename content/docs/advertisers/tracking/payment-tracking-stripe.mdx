...
Run `npx affitor setup stripe` to automatically configure Stripe webhooks via OAuth — no manual URL copy-paste or event selection needed. See the [CLI Quickstart](/advertisers/cli/quickstart/) for the full setup flow.
:::
...
When a tracked sale is refunded or disputed, report it so the associated commission is reversed before payout.
...
| Field | Required | Description |
|-------|----------|-------------|
| `transaction_id` | Yes | The `transaction_id` of the **original sale** you reported via the Sale API |
| `refund_amount_cents` | No | Partial refund amount. Omit for a full refund |
| `refund_reason` | No | Free-text reason, stored for audit |
...
- Reversal is idempotent: reporting the same refund twice does not double-reverse
- Returns `404` if no sale matches `transaction_id` for your program
- If the sale produced no commission, the call succeeds with `"status": "no_commission"`

See the full [Track Refund reference](/advertisers/api-reference/track-refund) for response schemas.

### Option B — Stripe webhook (Bill Flow)

If you use Stripe metadata + webhook tracking, `charge.refunded` events are processed automatically — refunds and disputes reverse the related commission with no API call needed on your side.
