---
title: Referrals Tracker (Lead Tracking)
description: Track signups and form submissions from affiliate traffic
---

Lead tracking captures when referred visitors sign up or register on your site. This is Step 2 of 3 in Affitor tracking (Click → **Lead** → Sale). Partners with CPL (cost-per-lead) commission models earn when leads are tracked.

**Prerequisite:** Click Tracking script must be installed first. [See Click Tracking setup →](/advertisers/tracking/pageview-tracker-click)

---

## What It Does

When a visitor signs up on your site, you queue a `trackLead` call via `window.affitorQueue`. This:

1. Validates that `user_id` is provided (critical for payment tracking)
2. Links the lead event to the stored `customer_code` from the click
3. Sends lead data to Affitor API for processing
4. Updates the customer-partner relationship status to "lead"

Partners can then see: Clicks → **Signups** → Sales

**Note:** In debug mode, calling `trackLead()` sends a test event instead of a real lead event. This helps verify your integration without creating actual lead records.

---

## Prerequisites

Before implementing lead tracking:

- [x] Pageview tracker installed ([guide](/advertisers/tracking/pageview-tracker-click))
- [x] Visitor arrived via affiliate link and has been tracked (creates `customer_code` in database)
- [x] `customer_code` cookie exists in browser (set automatically by pageview tracker)

**Important:** The lead event requires the `customer_code` cookie to link the signup to the affiliate partner. If this cookie is missing or the customer wasn't tracked via an affiliate link, the lead tracking will fail silently (no error, but lead won't be attributed).

---

## Basic Implementation

After your signup form submits successfully, call:

```javascript
window.affitorQueue = window.affitorQueue || [];
window.affitorQueue.push(['trackLead', {
  email: 'user@example.com',     // REQUIRED: user email (hashed before storage)
  user_id: 'user_123'            // REQUIRED: your internal user ID
}]);
```

:::tip[Program ID]
When you copy code from your Affitor dashboard (**Settings → Tracking**), your Program ID is pre-filled. If copying from these docs, replace `YOUR_PROGRAM_ID` with your ID from Settings.
:::

:::caution[Required for Payment Tracking]
The `user_id` parameter is **required** for payment attribution to work. This must be your internal user identifier that you'll also pass in Stripe checkout metadata. Without it, purchases cannot be linked to affiliate partners.
:::

---

## Full Implementation

Include additional data for better analytics:

```javascript
window.affitorQueue = window.affitorQueue || [];
window.affitorQueue.push(['trackLead', {
  email: 'user@example.com',        // REQUIRED: user email (hashed before storage)
  user_id: 'user_123',              // REQUIRED: your internal user ID
  name: 'John Doe',                 // Optional: discarded per privacy policy
  phone: '+1234567890',             // Optional: discarded per privacy policy
  additional_data: {
    signup_method: 'email',          // Optional: how they signed up
    form_type: 'registration',       // Optional: form type
    source: 'pricing_page'           // Optional: where they signed up
  }
}]);
```

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `email` | string | **Required** | User's email address (required for CPL partner tracking, hashed before storage) |
| `user_id` | string | **Required** | Your internal user ID - critical for payment attribution |
| `name` | string | No | User's full name (**NOT stored** - discarded per privacy policy) |
| `phone` | string | No | User's phone number (**NOT stored** - discarded per privacy policy) |
| `additional_data` | object | No | Any extra data you want to track (stored as-is) |

**Privacy Notes:**
- `email` is hashed (SHA-256) and masked before storage - the original email is never stored
- `name` and `phone` are **intentionally discarded** by the backend and never stored
- Only `email_hash`, `email_masked`, and `user_id` are persisted in the database

---

## Integration Examples

### Standard Form

```javascript
document.getElementById('signup-form').addEventListener('submit', async function(e) {
  e.preventDefault();

  const email = document.getElementById('email').value;
  const name = document.getElementById('name').value;

  // Your signup logic
  const result = await createAccount(email, name);

  if (result.success) {
    // Track the lead
    window.affitorQueue = window.affitorQueue || [];
    window.affitorQueue.push(['trackLead', {
      email: email,              // REQUIRED: user email
      user_id: result.userId,    // REQUIRED: your internal user ID
      name: name,                // Optional: discarded by backend
      additional_data: {
        signup_method: 'form',
        form_type: 'registration'
      }
    }]);

    // Redirect to dashboard or next step
    window.location.href = '/welcome';
  }
});
```

### React

```tsx
function SignupForm() {
  const handleSubmit = async (e) => {
    e.preventDefault();
    const formData = new FormData(e.target);

    // Your signup logic
    const result = await api.createAccount({
      email: formData.get('email'),
      name: formData.get('name')
    });

    if (result.success) {
      // Track the lead
      (window as any).affitorQueue = (window as any).affitorQueue || [];
      (window as any).affitorQueue.push(['trackLead', {
        email: formData.get('email'),    // REQUIRED: user email
        user_id: result.userId,          // REQUIRED: your internal user ID
        name: formData.get('name'),      // Optional: discarded by backend
        additional_data: {
          signup_method: 'react_form',
          form_type: 'registration'
        }
      }]);

      router.push('/welcome');
    }
  };

  return (
    <form onSubmit={handleSubmit}>
      <input name="email" type="email" required />
      <input name="name" type="text" />
      <button type="submit">Sign Up</button>
    </form>
  );
}
```

### Google/OAuth Signup

```javascript
// After OAuth callback
async function handleOAuthCallback(user) {
  // User authenticated via Google/GitHub/etc.

  // Track the lead
  window.affitorQueue = window.affitorQueue || [];
  window.affitorQueue.push(['trackLead', {
    email: user.email,       // REQUIRED: user email
    user_id: user.id,        // REQUIRED: your internal user ID
    name: user.name,         // Optional: discarded by backend
    additional_data: {
      signup_method: 'google',
      form_type: 'oauth'
    }
  }]);

  // Continue to dashboard
  window.location.href = '/dashboard';
}
```

### Tracking Plan Selection

If you want to track which plan users select during signup:

```javascript
window.affitorQueue = window.affitorQueue || [];
window.affitorQueue.push(['trackLead', {
  email: user.email,           // REQUIRED: user email
  user_id: user.id,            // REQUIRED: your internal user ID
  additional_data: {
    signup_method: 'email',
    form_type: 'registration',
    source: 'pricing_page',
    plan: 'free_trial'          // Optional: plan they selected
  }
}]);
```

---

## When to Call trackLead()

Call `trackLead()` **after** the signup is confirmed successful:

| Scenario | When to track |
|----------|---------------|
| Form signup | After form validation passes and account is created |
| OAuth signup | After OAuth callback, user authenticated |
| Email verification | After signup, not after verification (track intent) |
| Free trial | When trial starts |
| Waitlist | When added to waitlist |

**Don't track:**
- Failed signups
- Duplicate signups
- Bot submissions

---

## Why Use the Queue Pattern

The tracker script loads asynchronously. If your code runs before the script finishes loading, `window.affitor` will be `undefined` and the call silently fails — even with optional chaining (`window.affitor?.trackLead()`). This is a common issue in OAuth callback flows where `trackLead` runs immediately after redirect.

The queue pattern (`window.affitorQueue`) solves this by buffering commands. When the SDK loads, it automatically flushes the queue and executes all pending calls. This works reliably whether the SDK loads before or after your code runs.

```javascript
// Always works — before or after SDK loads
window.affitorQueue = window.affitorQueue || [];
window.affitorQueue.push(['trackLead', { email: email, user_id: userId }]);
```

:::note
Do **not** use `window.affitor?.trackLead()` directly — it silently fails if the SDK hasn't loaded yet. Always use the queue pattern above.
:::

---

## Verifying Lead Tracking

### Test Mode

:::tip[Debug Mode]
Enable debug mode in your Click Tracking script by setting `data-affitor-debug="true"`. See [Click Tracking → Test Mode](/advertisers/tracking/pageview-tracker-click#test-mode) for details.
:::

When debug mode is enabled in the pageview tracker:

```html
<script
  src="https://api.affitor.com/js/affitor-tracker.js"
  data-affitor-program-id="YOUR_PROGRAM_ID"
  data-affitor-debug="true">
</script>
```

**What happens when you call `trackLead()` in debug mode:**

1. The tracker sends a **test event** instead of a real lead event
2. No actual lead record is created in the database
3. Console shows validation messages for `user_id`
4. You'll see these console messages:

**With `user_id` provided:**
```
[Affitor Tracker] ✅ Affitor: user_id detected - user_123
[Affitor Tracker] trackTest() called true
[Affitor Tracker] Sending test event to verify tracking: {customerCode: ..., hasAttribution: true, eventType: "test"}
```

**Without `user_id`:**
```
[Affitor Tracker] ❌ Affitor: user_id is REQUIRED for trackLead() - payment attribution will not work without it!
[Affitor Tracker]    Please include user_id in your trackLead call:
[Affitor Tracker]    window.affitor.trackLead({ email: "user@example.com", user_id: "user_123", ... })
[Affitor Tracker] trackTest() called true
```

**Important:** In debug mode, `trackLead()` always sends a test event and returns immediately without calling the real API. This prevents accidental test leads in your database.

### Dashboard Verification

1. Visit your site
2. Go to Affitor dashboard → **Setting** → **Referrals Tracker**
3. Click on **Test Lead Tracking**
4. Referrals Tracker should show **"Connected"** with a blue checkmark

---

## Troubleshooting

### trackLead() Not Working

**Check:**
- `window.affitor` exists (pageview tracker loaded)
- `customer_code` cookie exists (visitor came via affiliate link)
- No JavaScript errors in console
- Not in debug mode (debug mode sends test events only)

### Lead Not Attributed to Partner

**Check:**
- Visitor came via affiliate link (`?ref=PARTNER123`) **before** signing up
- `customer_code` cookie exists and matches a tracked click in the database
- Attribution window hasn't expired (60 days default)
- Affiliate customer record exists in database (created during click tracking)

**Common issue:** If the visitor's first page load didn't include `?ref=`, no customer record was created, so the lead cannot be attributed even if cookies exist.

### user_id Warning Appears

**This is expected behavior** when `user_id` is missing:

```
❌ Affitor: user_id is REQUIRED for trackLead() - payment attribution will not work without it!
```

**Solutions:**
- Always include `user_id` in your `trackLead()` call
- `user_id` should be your internal user identifier (from your database)
- The same `user_id` must be used in Stripe checkout metadata

**Note:** The lead is still tracked even without `user_id`, but payment attribution will fail later.

### Duplicate Leads

**Prevent by:**
- Only calling `trackLead()` once per signup
- Checking signup success before tracking
- Backend automatically handles duplicate lead tracking (updates existing customer instead of creating new lead)

### Lead Tracked But Not Showing in Dashboard

**Check:**
- Lead event was successfully sent (check browser Network tab)
- Customer-partner relationship exists in database
- Lead status updated to "lead" (was previously "click")
- Dashboard filters aren't hiding the lead

---

**Next:** [Set up Sale Tracking →](/advertisers/tracking/payment-tracking-stripe)
