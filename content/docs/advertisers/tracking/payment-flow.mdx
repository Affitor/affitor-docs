...
- **`customerKey` saved to customer-partner relationship**
...
- Reads `customer_key` from metadata
- Reads `program_id` from metadata
- Reads `affitor_click_id` from metadata
- Looks up customer-partner relationship using `customer_key`, `program_id` and `affitor_click_id`
