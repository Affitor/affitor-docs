...
- **Server** (`@affitor/sdk/server`): `await affitor.trackLead({ customerExternalId, clickId?, email? })` with a Bearer program API key.
...
- **Server-side tracking** (`@affitor/sdk/server` or raw): `POST /api/v1/track/sale`, **Bearer REQUIRED**.
  ```ts
...
1. **Package name.** The published package is **`@affitor/sdk`** — browser `init`/`signup` (the default entry), `/react` `AffitorProvider`, and `/server` `Affitor`. It consolidates the former `affitor-sdk` / `affitor-node` / `@affitor/tracker` packages into one. Ensure `api-reference/sdks.mdx`, `index.mdx`, `agent-integration.mdx`, and anywhere else reference `@affitor/sdk` (+ `/server`, `/react`), never the legacy names.
...
6. **Node / Express** server — `@affitor/sdk/server` for lead + sale, raw API fallback.
...
- [ ] Uses ONLY §2 verified ingredients; package name = `@affitor/sdk` (+ `/server`, `/react`).
